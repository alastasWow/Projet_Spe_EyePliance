...
KEY=AIzaSyAvU69Zckq4qbmysRouS2SIKKVXJ0DiIn8 
...
FILTERED=`echo "$RESULT"| sed -e 's/[{""}]/''/g' | awk -F":" '{print $4}' | awk -F"," '{print $1}' | tr -d '\n'`
#| sed -e 's/[{""}]/''/g' | awk -F":" '{print $4}' | awk -F"," '{print $1}' | tr -d '\n'`
#| sed 's/,/\n/g;s/[{,},"]//g;s/\[//g;s/\]//g;s/:/: /g'` #|awk -F": " '{print $4}'`
#| sed 's/,/\n/g;s/[{,},"]//g;s/\[//g;s/\]//g;s/:/: /g' | grep -o -i -e "transcript.*"| awk -F": " '{print $2}'`

echo > $OUTFILE
if [[ ! "$FILTERED" ]]
  then
     >&2 echo "I was unable to recognize any speech in audio data"
else
